    ...
    <table class="border-collapse w-full">
      <thead :class="headerClass">
        <tr class="sm:rounded-lg">
          <th v-if="isCheckboxTable"
            class="font-bold leading-none uppercase bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-300 table-cell w-5"
            :class="[
              { 'p-1 text-xs': size == 'sm' },
          ...
          <th v-for="coluna in colunas.filter((col) => !col.abaixo)" :key="`coluna-${coluna.key}`"
            class="font-bold uppercase bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-300 table-cell"
            :class="[
              { 'p-1 text-xs': size == 'sm' },
              { 'p-3 text-sm': size == 'base' },
            ]">
      ...
      <tbody v-for="(item, index) in value" :key="item.ukey">
        <tr @click="emit('itemClick', { item, index })"
          class="bg-white dark:bg-black dark:hover:bg-gray-800 cursor-pointer hover:bg-gray-100 flex table-row flex-row flex-no-wrap mb-0"
          :class="itemScopedClass(item)">
          <td v-if="isCheckboxTable"
            class="leading-none w-auto text-gray-800 dark:text-gray-200 border border-b flex items-center table-cell static" 
          ...
          <td v-for="coluna in colunas.filter((col) => !col.abaixo)" :key="`coluna-${coluna.key}`"
            class="w-auto text-gray-800 dark:text-gray-200 border border-b flex items-center table-cell static"
